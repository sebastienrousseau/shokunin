...
fn main() -> Result<(), TemplateError> {
    println!("🦀 Comprehensive SSG Template Library Usage Examples 🦀\n");
...
fn demonstrates_template_rendering_example() -> Result<(), TemplateError> {
    ...
    context.set("body".to_string(), "This is a rendered page.".to_string());
...
fn demonstrates_error_handling_example() -> Result<(), TemplateError> {
    ...
    match engine.render_page(&context, "invalid") {  // Pass only "invalid", not "invalid.html"
...
fn demonstrates_template_with_conditionals_example() -> Result<(), TemplateError> {
    println!("\n🦀 Template with Conditionals Example 🦀");

    let mut context = Context::new();
    context.set("title".to_string(), "Conditional Rendering".to_string());
    ...
    let rendered_template = engine.render_page(&context, "conditional")?;
    println!("Rendered Template with Conditionals: ✅ \n{}", rendered_template);
...
fn demonstrates_nested_context_example() -> Result<(), TemplateError> {
    ...
    let mut context = Context::new();
    context.set("title".to_string(), "Nested Context Example".to_string());
    ...
    let rendered_template = engine.render_page(&context, "user_profile")?;  // Pass only "user_profile", not "user_profile.html"

    println!("Rendered Template with Nested Context: ✅ \n{}", rendered_template);

    Ok(())
}

/// This function demonstrates rendering a template with partials using the `Engine` struct.
fn demonstrates_template_with_partials_example() -> Result<(), TemplateError> {
    ...
    context.set("body".to_string(), "This is the main content.".to_string());
    ...
    header_file.write_all(b"<header><h1>Welcome to {{title}}</h1></header>")?;
    ...
    let main_template_path = template_dir_path.join("page_with_partials.html");
    ...
    let rendered_template = engine.render_page(&context, "page_with_partials")?;
    println!("Rendered Template with Partials: ✅ \n{}", rendered_template);

    Ok(())
}

/// Demonstrates template inheritance using a base layout and extending templates.
fn demonstrates_template_with_inheritance_example() -> Result<(), TemplateError> {
    ...
    context.set("header".to_string(), "Welcome to the About Page".to_string());
    ...
    println!("Rendered Template with Inheritance (Manual): ✅ \n{}", rendered_template);
...
fn demonstrates_multiple_templates_example() -> Result<(), TemplateError> {
    ...
    context_contact.set("email".to_string(), "contact@example.com".to_string());
    ...
    let rendered_contact = engine.render_page(&context_contact, "contact")?;
...
fn demonstrates_large_context_example() -> Result<(), TemplateError> {
    ...
    for i in 1..101 {
        ...
        template_content.push_str(&format!("<p>{{{{key{}}}}}</p>\n", i));
    }
    ...
    let rendered_template = engine.render_page(&context, "large_context")?;
    println!("Rendered Template with Large Context: ✅ \n{}", rendered_template);
...
fn demonstrates_dynamic_content_example() -> Result<(), TemplateError> {
    ...
    let rendered_template = engine.render_page(&context, "dynamic_content")?;
    println!("Rendered Template with Dynamic Content: ✅ \n{}", rendered_template);
...
fn demonstrates_template_caching_example() -> Result<(), TemplateError> {
    ...
    if let Some(cached_template) = template_cache.get("cached_template") {
        ...
        let rendered_template = engine.render_page(&context, "cached_template")?;
        template_cache.insert("cached_template".to_string(), rendered_template.clone());
        println!("Rendered and cached template: ✅ \n{}", rendered_template);
    }
...
fn demonstrates_template_debugging_example() -> Result<(), TemplateError> {
    ...
    match engine.render_page(&context, "debug_template") {
        Ok(rendered) => println!("Rendered Template: ✅ \n{}", rendered),
...
fn demonstrates_form_template_example() -> Result<(), TemplateError> {
    ...
    let rendered_template = engine.render_page(&context, "form_template")?;
...
fn demonstrates_custom_error_handling_example() -> Result<(), TemplateError> {
    println!("\n🦀 Custom Error Handling Example 🦀");

    let mut context = Context::new();
    context.set("title".to_string(), "Custom Error Example".to_string());
    ...
    match engine.render_page(&context, "nonexistent_template") {
        Ok(rendered) => println!("Unexpected success: \n{}", rendered),
        Err(e) => match e {
            TemplateError::Io(_) => println!("Custom Error: Template file not found! ❌"),
            TemplateError::RenderError(msg) => println!("Custom Render Error: {} ❌", msg),
...
fn demonstrates_pagination_example() -> Result<(), TemplateError> {
    ...
    let template_path = template_dir_path.join("pagination_template.html");
    let mut template_file = File::create(&template_path)?;
    let template_content = r#"
        <html>
        <head><title>{{title}}</title></head>
    ...
    let rendered_template = engine.render_page(&context, "pagination_template")?;
...
fn demonstrates_internationalization_example() -> Result<(), TemplateError> {
    ...
    if lang == "fr" {
        context.set("greeting".to_string(), "Bonjour le monde".to_string());
    } else {
        context.set("greeting".to_string(), "Hello World".to_string());
    }
    ...
    let rendered_template = engine.render_page(&context, "i18n_template")?;
...
fn demonstrates_template_fragments_example() -> Result<(), TemplateError> {
    ...
    let main_template_path = template_dir_path.join("page_with_fragments.html");
    let mut main_template_file = File::create(&main_template_path)?;
    let main_template_content = r#"
        {{header}}
    ...
    let rendered_header = engine.render_page(&Context::new(), "header")?;
    let rendered_footer = engine.render_page(&Context::new(), "footer")?;
    ...
    let rendered_template = engine.render_page(&context, "page_with_fragments")?;
    println!("Rendered Template with Fragments: ✅ \n{}", rendered_template);
...
fn demonstrates_markdown_to_html_example() -> Result<(), TemplateError> {
    ...
    let template_path = template_dir_path.join("markdown_template.html");
    let mut template_file = File::create(&template_path)?;
    let template_content = r#"
        <html>
    ...
    let rendered_template = engine.render_page(&context, "markdown_template")?;
    println!("Rendered Markdown to HTML (Manual Conversion): ✅ \n{}", rendered_template);

    Ok(())
}

/// Demonstrates template versioning by switching between multiple versions of a template.
fn demonstrates_template_versioning_example() -> Result<(), TemplateError> {
    ...
    let template_version = if client_version == 1 { "v1" } else { "v2" };

    // Initialize the Engine with the template directory
    let engine = Engine::new(template_dir_path.to_str().unwrap());

    // Render the appropriate version of the template
    let rendered_template = engine.render_page(&context, template_version)?;
    println!("Rendered Template Version {}: ✅ \n{}", client_version, rendered_template);
...
fn demonstrates_ajax_template_example() -> Result<(), TemplateError> {
    ...
    let mut context = Context::new();
    context.set("message".to_string(), "This content is dynamically loaded.".to_string());
    ...
    let rendered_template = engine.render_page(&context, "ajax_template")?;
